...
def index(request):
    ...
    dvr = {'allProds': allProds}
    return render(request, 'shop/index.html', dvr)
...
def search(request):
    ...
    for cat in cats:
        ...
        if len(prod) != 0:
            allProds.append([prod, range(1, nSlides), nSlides])
    dvr = {'allProds': allProds, "msg": ""}
    if len(allProds) == 0 or len(query) < 3:
        dvr = {'msg': "No item available. Please make sure to enter relevant search query"}
    return render(request, 'shop/search.html', dvr)
